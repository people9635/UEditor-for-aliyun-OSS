...
public class Uploader {
	...
	public final State doExec(String appendRootPath) {
		String filedName = (String) this.conf.get("fieldName");
		State state = null;

		if ("true".equals(this.conf.get("isBase64"))) {
			state = Base64Uploader.save(appendRootPath,this.request.getParameter(filedName),
					this.conf);
		} else {
			state = BinaryUploader.save(appendRootPath,this.request, this.conf);
			JSONObject stateJson = new JSONObject(state.toJSONString());
			// 判别云同步方式
			if (OSSClientProperties.useStatus) {
